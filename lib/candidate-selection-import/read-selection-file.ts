...
/// header/row grid. Unlike the live candidate import (CSV only), this file
/// type is explicitly XLSX-or-CSV per the Phase 15 brief.
...
/// handle regardless of source format.
...
/// Reads only the first worksheet (brief §21: "use the first worksheet unless
/// the existing requirements specify a named worksheet" — no such requirement
/// exists). A workbook with additional sheets is not an error; they are simply
/// not read.
...
export async function readSelectionFile(file: File): Promise<ReadResult> {
